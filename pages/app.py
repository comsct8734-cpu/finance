...
if not full_loaded:
    st.warning("⚠️ KRX 전체 종목 목록을 불러오지 못해 주요 종목 40여 개로 검색합니다. (KRX 서버가 일시적으로 응답하지 않을 때 발생하며, 잠시 후 새로고침하면 전체 목록이 복구될 수 있습니다.)")
...
if do_predict:
    ...
    try:
        ...
        st.info(f"최근 {fit_window}일 데이터로 그린 추세선이 과거를 설명하는 정도(R²)는 {r2:.2f} 입니다. (1에 가까울수록 직선 추세가 뚜렷, 0에 가까울수록 들쭉날쭉해서 직선으로 설명하기 어려움)")

        st.caption("※ 선형회귀는 '최근 흐름이 직선으로 계속된다'고 가정하는 가장 단순한 예측입니다. 실제 주가는 곡선·급변동이 많아 이 가정이 자주 깨집니다. 바로 이 한계를 관찰하는 것이 이 프로젝트의 중요한 학습 포인트입니다.")
    except Exception as e:
        st.error(f"예측 중 오류가 발생했습니다: {e}")

# ── 8. 하단 교육적 장치 (Disclaimer) ──
st.markdown("---")
st.error("⚠️ 교육용 안내 (반드시 읽어주세요)")
st.markdown("- 이 앱은 **학습 및 탐구 목적의 교육용 프로젝트**이며, **투자 권유나 금융 조언이 아닙니다.**")
st.markdown("- 예측값은 과거 데이터의 추세를 직선으로 추정한 결과일 뿐, **미래의 실제 주가를 보장하지 않습니다.**")
st.markdown("- 주가는 경제 상황, 기업 실적, 뉴스, 투자 심리 등 수많은 요인의 영향을 받아 어떤 모델도 정확히 예측할 수 없습니다.")
st.markdown("- 실제 투자는 본인의 신중한 판단과 책임 하에 이루어져야 하며, **이 앱의 결과를 투자 근거로 사용하지 마세요.**")
st.markdown("- 데이터 출처: 한국거래소(KRX) / FinanceDataReader")
